    ...
    auto __f = [&] (size_t _from, size_t _to) -> void {
        ...
        for(size_t i = _from ; i < _to;  ++i) {
            ...
            if (typeid(*p_rep) == typeid(basic_packet4)) {
                ...
                const auto _stack_code = convert_pkt_code_to_stack(_p_rep->tp);
                const auto _flow_id = extend_tuple(_p_rep->flow_id, _stack_code);
...
void explicit_flow_constructor::dump_flow_statistic(void) const {
    ...
    for (auto p_flow: *p_construct_result4) {
        for (uint8_t i = 0; i < stack_type_t::F_UNKNOWN; i ++) {
            if (tuple_get_stack(p_flow->flow_id) == to_stack_code((stack_type_t) i)) {
                __sat4[i] ++;
            }
        }
    }
    ...
    for (size_t i = 0; i < stack_type_t::F_UNKNOWN; i ++) {
        printf("[%-8s]: %d\n", stack_type_names[i], __sat4[i]);
    }
    printf("[Sum IPv6 Flow]: %ld\n", p_construct_result6->size());
    for (size_t i = 0; i < stack_type_t::F_UNKNOWN; i ++) {
        printf("[%-8s]: %d\n", stack_type_names[i], __sat6[i]);
    }
...
std::ostream& operator<<(std::ostream& os, const tuple5_conn4& flow_id) {
    const auto& [src_ip, dst_ip, src_port, dst_port, stack_code] = flow_id;

    os << get_str_addr(src_ip) << " "
       << get_str_addr(dst_ip) << " "
       << src_port << " "
       << dst_port << " "
       << stack_type_names[stack_code_to_type(stack_code)];
    return os;
}

std::ostream& operator<<(std::ostream& os, const tuple5_conn6& flow_id) {
    const auto& [src_ip, dst_ip, src_port, dst_port, stack_code] = flow_id;
...
std::string to_string(const tuple5_conn4& flow_id) {
    const auto& [src_ip, dst_ip, src_port, dst_port, stack_code] = flow_id;

    std::ostringstream oss;
    oss << get_str_addr(src_ip) << " "
       << get_str_addr(dst_ip) << " "
       << src_port << " "
       << dst_port << " "
       << stack_type_names[stack_code_to_type(stack_code)];
...
void explicit_flow_constructor::dump_flow_fct_to_file(void) const {
    __START_FTIMMER__
    ...
    // 打开文件
    std::ofstream all_fct("all_flow_fct.txt");
    std::ofstream tcp_fct("tcp_flow_fct.txt");
    std::ofstream udp_fct("udp_flow_fct.txt");

    if (!all_fct || !tcp_fct || !udp_fct) {
        FATAL_ERROR("Failed to open FCT output files.");
    }

    // 写入 FCT 数据的函数
    auto write_fct_data = [&](const auto& flows) {
        for (const auto& flow : flows) {
            if (!flow) continue;
            double fct = flow->get_fct();

            all_fct << std::fixed << std::setprecision(6)
                    << flow->flow_id << " " 
                    << flow->get_str_time() << " " 
                    << flow->get_end_time() << " " 
                    << fct << " " 
                    << flow->get_p_packet_p_seq()->size() << " "
                    << flow->get_flow_length() << "\n";
        }
    };

    // 写入 IPv4 和 IPv6 的 FCT 数据
    write_fct_data(*p_construct_result4);
    write_fct_data(*p_construct_result6);
...
void explicit_flow_constructor::dump_flow_fct_to_file(const shared_ptr<binary_label_t> &p_label) const {
    __START_FTIMMER__
    ...
    // 打开文件
    std::ofstream all_flow("all_flow.txt");
    ...
    auto write_fct_data = [&](const auto& flows) {
        for (const auto& flow : flows) {
            if (!flow) continue;
            double fct = flow->get_fct();
            bool is_malicious = flow->get_flow_label(p_label);

            all_flow << std::fixed << std::setprecision(6)
                    << flow->flow_id << " " 
                    << flow->get_str_time() << " " 
                    << flow->get_end_time() << " " 
                    << fct << " " 
    ...
    // === 随机抽样校验部分 ===
    std::ofstream verify_out("verify_flows.txt");
    if (!verify_out) {
        FATAL_ERROR("Failed to open verify_flows.txt for writing.");
    }

    size_t check_count = p_construct_result4->size();

    LOGF("\n[IPv4 Flow Label Verification Sample: %zu flows]", check_count);
    for (size_t i = 0; i < check_count; ++i) {
        const auto& flow = p_construct_result4->at(i);

        std::ostringstream label_stream;
        std::ostringstream index_stream;
        for (size_t idx : *(flow->get_p_reverse_id())) {
            index_stream << idx << ",";
            if (idx >= p_label->size()) {
                label_stream << "?,";
            } else {
                label_stream << ((*p_label)[idx] ? "1," : "0,");
            }
        }

        bool label_result = flow->get_flow_label(p_label);

        // ====== 文件写入 ======
        verify_out << "Flow ID: " << to_string(flow->flow_id) << "\n";
        verify_out << "Packet Indices: " << index_stream.str() << "\n";
        verify_out << "Packet Labels : " << label_stream.str() << "\n";
        verify_out << "get_flow_label(): " << (label_result ? "true" : "false") << "\n";
        verify_out << "---------------------------------------------------\n";
    }
